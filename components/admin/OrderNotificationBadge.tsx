...
export default function OrderNotificationBadge() {
  const { newOrdersCount, resetNewOrdersCount, soundArmed, needsSoundActivation, armSound } = useOrderNotifications({
    enabled: true,
  ...
  // Quand le son n'est pas armé, on affiche un petit bouton permanent.
  const showSoundActivator = !soundArmed;

  if ((!showBadge || newOrdersCount === 0) && !showSoundActivator) return null;
    ...
    <>
      {showSoundActivator && (
        <div className="fixed bottom-4 right-4 z-50">
          <button
            onClick={armSound}
            className={`px-4 py-3 rounded-xl shadow-lg font-semibold transition-colors ${
              needsSoundActivation
                ? 'bg-red-600 hover:bg-red-700 text-white'
                : 'bg-gray-900 hover:bg-black text-white'
            }`}
            title="Cliquez une fois pour autoriser le son (restriction navigateur)."
          >
            🔊 Activer le son
          </button>
        </div>
