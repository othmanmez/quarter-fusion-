...
async function main() {
  ...
  const baseData = {
    title,
    description: 'Tacos (3 viandes max) - avec frites',
    price: 10.0,
    priceClickAndCollect: 10.0,
    priceDelivery: 11.5,
    image: '/images/tacos-3-viandes.png',
    categoryId: cat.id,
    available: true,
  ...
  // Viandes: choix multiple, max 3, obligatoire (au moins 1 sélection)
  await upsertCustomization({
    menuId: menu.id,
