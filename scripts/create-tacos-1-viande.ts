...
async function main() {
  ...
  const baseData = {
    title,
    description:
      'Tacos 1 viande - avec frites et 1 boisson 33 cl',
    price: 6.5,
    priceClickAndCollect: 6.5,
    priceDelivery: 7.5,
    image: '/images/tacos-1-viande.png',
    categoryId: cat.id,
    available: true,
  ...
  // Boisson : non obligatoire, supplément +1,20€ si sélectionné
  await prisma.customization.deleteMany({
  ...
  await upsertCustomization({
    ...
    required: false,
    maxSelections: null,
    // on réutilise les boissons 33cl du site si elles existent, sinon fallback minimal
    options: (
      await (async () => {
