...
async function main() {
  const sandwichCategory = await prisma.category.findUnique({
    where: { slug: 'sandwichs-normaux' },
  });
  if (!sandwichCategory) throw new Error("Catégorie 'sandwichs-normaux' introuvable");
  ...
  // Suppléments (non obligatoires) - mêmes blocs que les sandwichs au four
  ...
  console.log(`Sandwichs normaux trouvés: ${sandwiches.length}`);

  for (const s of sandwiches) {
    // Éviter l'ancienne option boisson "supplémentaire"
